...
export class ComputeClient {
    ...
    async deploy(options) {
        ...
        return Result.gen(async function* () {
            ...
            const artifact = yield* Result.await(Result.tryPromise({
                try: () => options.strategy.execute(options.signal),
                catch: (e) => new BuildError({
            ...
            try {
                ...
                const archiveBytes = yield* Result.await(Result.tryPromise({
                    try: () => createArchive(artifact.directory, artifact.entrypoint, options.signal),
    ...
    async createProject(options) {
        ...
        return Result.gen(async function* () {
            ...
            const response = yield* Result.await(self.#api.createProject({
                name: options.name,
                createDatabase,
            ...
            if (response.database) {
                ...
                result.database = {
                    ...
                    connectionString: connection?.endpoints?.direct?.connectionString ?? undefined,
                };
            }
    ...
    async listProjects(options = {}) {
        ...
        return Result.gen(async function* () {
            yield* self.#checkAborted(options.signal);
            const projects = yield* Result.await(self.#api.listProjects(options.signal));
            return Result.ok(projects.map((p) => ({
                id: p.id,
                name: p.name,
    ...
    async #resolveDeployTarget(options) {
        ...
        return Result.gen(async function* () {
            ...
            const region = options.region ?? (await options.interaction?.selectRegion?.(REGIONS));
            const response = yield* Result.await(self.#api.createProjectApp(projectId, {
                displayName: appName,
                ...(region !== undefined ? { regionId: region } : {}),
            }, options.signal));
            return Result.ok({
                projectId,
                appId: response.id,
    ...
    async #applyEnvVars(projectId, branchId, envVars, signal) {
        if (!envVars || Object.keys(envVars).length === 0) {
            return Result.ok(undefined);
        }
        const self = this;
        return Result.gen(async function* () {
            const scope = yield* Result.await(self.#resolveEnvironmentVariableScope(projectId, branchId, signal));
            for (const [key, value] of Object.entries(envVars)) {
                yield* self.#checkAborted(signal);
                const existing = yield* Result.await(self.#api.listEnvironmentVariables({ ...scope, key }, signal));
                const current = existing[0];
                if (value === null) {
                    if (current) {
                        yield* self.#checkAborted(signal);
                        // Do not pass signal to mutating transport; aborting it can make remote completion ambiguous.
                        yield* Result.await(self.#api.deleteEnvironmentVariable(current.id));
                    }
                    continue;
                }
                if (current) {
                    yield* self.#checkAborted(signal);
                    // Do not pass signal to mutating transport; aborting it can make remote completion ambiguous.
                    yield* Result.await(self.#api.updateEnvironmentVariable(current.id, { value }));
                    continue;
                }
                yield* self.#checkAborted(signal);
                // Do not pass signal to mutating transport; aborting it can make remote completion ambiguous.
                yield* Result.await(self.#api.createEnvironmentVariable({ ...scope, key, value }));
            }
            return Result.ok(undefined);
        });
    }
    async #resolveEnvironmentVariableScope(projectId, branchId, signal) {
        if (!branchId) {
    ...
    async #uploadArtifactResult(url, body, signal) {
...
//# sourceMappingURL=compute-client.js.map