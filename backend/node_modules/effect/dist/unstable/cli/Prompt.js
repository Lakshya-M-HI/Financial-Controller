...
const handleConfirmClear = options => {
  return Effect.fnUntraced(function* (state, _) {
    ...
    const promptText = renderConfirmOutput(confirmMessage, options.prefix, figures.pointerSmall, options, {
      plain: true
    });
...
const handleDateClear = options => {
  return Effect.fnUntraced(function* (state, _) {
    ...
    const promptText = renderDateOutput(options.prefix, figures.pointerSmall, parts, options, {
      plain: true
    });
...
const handleFileClear = options => {
  return Effect.fnUntraced(function* (state, _) {
    ...
    const promptText = isConfirming ? renderPrompt("(Y/n)", CONFIRM_MESSAGE, options.prefix, figures.pointerSmall, {
      plain: true
...
const handleMultiSelectClear = options => Effect.fnUntraced(function* (state, _) {
  ...
  const promptText = renderSelectOutput(options.prefix, figures.pointerSmall, options, {
    plain: true
  });
...
const handleNumberClear = options => {
  return Effect.fnUntraced(function* (state, _) {
    ...
    const promptText = renderNumberOutput(state, options.prefix, figures.pointerSmall, options, {
      plain: true
    });
    const clearOutput = clearOutputWithError(promptText, columns, errorText);
    return clearOutput + resetCurrentLine;
...
const handleAutoCompleteClear = options => Effect.fnUntraced(function* (state, _) {
  ...
  const figures = yield* platformFigures;
  const clearPrompt = Ansi.eraseLine + Ansi.cursorLeft;
  const promptText = renderAutoCompleteOutput(state, options.prefix, figures.pointerSmall, options, {
    plain: true
  });
...
const renderClearScreen = /*#__PURE__*/Effect.fnUntraced(function* (state, options) {
  ...
  const clearOutput = clearOutputWithError(renderTextOutput(state, options.prefix, figures.pointerSmall, options, {
    plain: true
  }), columns, errorText);
  return clearOutput + resetCurrentLine;
});
...
const handleToggleClear = /*#__PURE__*/Effect.fnUntraced(function* (options) {
  ...
  const promptText = renderPrompt(toggleText, options.message, options.prefix, figures.pointerSmall, {
    plain: true
  });
  const clearOutput = eraseText(promptText, columns);
  return clearOutput + clearPrompt;
